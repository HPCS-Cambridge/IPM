...
/* AT: TODO - place double pointer at optimised location? */
...
// AT: TODO - safe memory allocation
/* Set timestamps[count-1] to tstart_ to store function call time, t_ for call
 * duration. Or set timestamps to a struct of 2 doubles and store both.
 */
// AT: TODO - use only realloc?
#define IPM_HASHTABLE_ADD(idx_,t_,tstart_)				\
  {								\
    if( idx_>= 0 && idx_ < MAXSIZE_HASH ) {			\
      ...
      if( ipm_htable[idx_].timestamps ) {   \
        ...
        ipm_htable[idx_].timestamps = malloc(sizeof(double) * ipm_htable[idx_].count);    \
      }   \
      if( !ipm_htable[idx_].timestamps ) {                                \
        abort();                                                          \
      }                                                                   \
