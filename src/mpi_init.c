...
void ipm_mpi_init() {
  ...
  ipm_interval_htable[htable_switch][idx].count++;
  ...
  if( ipm_htable[idx].timestamps) {
    ...
    ipm_interval_htable[htable_switch][idx].timestamps = realloc(ipm_interval_htable[htable_switch][idx].timestamps, sizeof(double) * ipm_htable[idx].count);
  }
  else {
    ipm_htable[idx].timestamps = malloc(sizeof(double) * ipm_htable[idx].count);
    ipm_interval_htable[htable_switch][idx].timestamps = malloc(sizeof(double) * ipm_htable[idx].count);
  }
  if( !ipm_htable[idx].timestamps || !ipm_interval_htable[htable_switch][idx].timestamps ) {
    abort();
  }
  IPM_TIMESTAMP(ipm_htable[idx].timestamps[ipm_htable[idx].count-1]);
  ipm_interval_htable[htable_switch][idx].timestamps[ipm_htable[idx].count-1] = ipm_htable[idx].timestamps[ipm_htable[idx].count-1];
  ipm_interval_htable[htable_switch][idx].t_min=0.0;
  ipm_interval_htable[htable_switch][idx].t_max=0.0;
  ipm_interval_htable[htable_switch][idx].t_tot=0.0;
  // END AT
