...
ipm_hent_t ipm_interval_htable[2][MAXSIZE_HASH] = {0};
		 ...
		 scanspec_t spec )
{
  ...
  stats->bytesum=0.0;
  stats->hent.count=0;
  /* AT: TODO - free outside function call*/
  stats->hent.timestamps = NULL;
  ...
  for( i=0; i<MAXSIZE_HASH; i++ )
    ...
    // AT: TODO clean allocs
    if( stats->hent.timestamps ) {
      stats->hent.timestamps = realloc(stats->hent.timestamps, sizeof(double) * stats->hent.count);
    }
    else {
      stats->hent.timestamps = malloc(sizeof(double) * stats->hent.count);
    }
    for( j = oldcount; j < stats->hent.count; j++ ) {
      stats->hent.timestamps[j] = table[i].timestamps[j-oldcount];
    }
    // END AT
  ...
  for( i=0; i<MAXSIZE_HASH; i++ )
      ...
      for( j=0; j<nspec; j++ )
	{
	  ...
	  if( (lact<=act && act<=hact) &&
	    ...
	    {
	      res++;
        oldcount = stats[j].count;
	      stats[j].count+=table[i].count;
	      stats[j].t_tot+=table[i].t_tot;

        // AT: TODO clean allocs
        if( stats[j].timestamps ) {
          stats[j].timestamps = realloc(stats[j].timestamps, sizeof(double) * stats[j].count);
        }
        else {
          stats[j].timestamps = malloc(sizeof(double) * stats[j].count);
        }
        if( !stats[j].timestamps ) {
          abort();
        }
        for( k = oldcount; k < stats[j].count; k++ ) {
          stats[j].timestamps[k] = table[i].timestamps[k-oldcount];
        }
        // END AT
  ...
  for( i=0; i<MAXSIZE_HASH; i++ )
    {
	  ...
	  (lbyt<=byt && byt<=hbyt) )
	{
    oldcount = stats[act].count;
	  stats[act].count+=table[i].count;
	  stats[act].t_tot+=table[i].t_tot;

    // AT: TODO clean allocs
    if( stats[act].timestamps ) {
      stats[act].timestamps = realloc(stats[act].timestamps, sizeof(double) * stats[act].count);
    }
    else {
      stats[act].timestamps = malloc(sizeof(double) * stats[act].count);
    }
    for( k = oldcount; k < stats[act].count; k++ ) {
      stats[act].timestamps[k] = table[i].timestamps[k-oldcount];
    }
    // END AT
